...
**Outlier Removal:** Outlier removal was done in multiple stages using percentiles. Prices were grouped by Currency (AUD, USD) and Rental Duration ('Monthly', 'Daily') to assess them within their distribution. Floor area and price outliers were found using a 5% upper and lower percentile (or 90% IQR - Inter Quartile Range). An additional round of outlier filtering was performed on the normalized price, this time removing values outside a 1st to 99th percentile to retain broader coverage while excluding extreme cases. The choice of thresholds was informed by trial and error and remains subjective.
